...
- 直接用 `DeviceOrientation` 的 `gamma`/`beta`(角度制)规定剑的朝向: gamma 管左右、beta 管上下。
- **9 方向引导校准**(正前/左/右/上/下/四对角): 每个方向采样 `(gamma,beta)` 存入 3×3 校准网格。
- 游玩时实时读数在 9 个校准点间分段线性插值 → 音符场坐标 → 剑指向该点。指向哪, 剑指哪。
