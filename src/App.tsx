...
const STORAGE_KEYS = {
  ...
  history: "ai_chat_pro_history", // 新增：保存对话历史
};

function App() {
  // --- Core States ---
  // 修复1：从 LocalStorage 加载历史对话
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      while (!doneReading) {
        ...
        // 修复2：确保 value 存在才进行 decode，防止 undefined 错误
        if (value) {
                ...
                if (contentChunk || reasoningChunk) {
                  setHistory((prev) => {
                    ...
                    if (lastIdx >= 0 && h[lastIdx].role === "assistant") {
                      ...
                      if (contentChunk) {
                        h[lastIdx].content =
                          (h[lastIdx].content as string) + contentChunk;
                      }
                      // Update reasoning content (thinking process)
                      if (reasoningChunk) {
                        h[lastIdx].reasoning_content =
                          (h[lastIdx].reasoning_content || "") + reasoningChunk;
                      }
                    }
                    return h;
                  });
  ...
  // 修复3：自定义 Markdown 组件，添加横向滚动条支持
  const markdownComponents = {
    pre: ({ children, ...props }: any) => (
      <pre 
        className="overflow-x-auto max-w-full p-4 bg-gray-950 rounded-lg border border-gray-700" 
        {...props}
      >
        {children}
      </pre>
    ...
    code: ({ children, inline, ...props }: any) => (
      inline ? (
        <code className="bg-gray-800 px-1.5 py-0.5 rounded text-sm font-mono text-indigo-300" {...props}>
          {children}
        </code>
      ) : (
