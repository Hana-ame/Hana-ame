...
// 注: ahrs 包的 kp/ki 是构造常量, 运行期重建实例会丢失积分器与姿态基准, 故 ahrs 版用固定增益。
// (adaptiveGains 保留仅供自研版 lib/attitude.js 对照使用)
...
export function initPc() {
  ...
  function onSensor(snap) {
    ...
    els.fusionNote.textContent = inited
      ? `状态: 固定增益 Mahony (Kp 0.5, Ki 0.05, 积分消零偏) · `
        + `偏航${yawFlip ? '反转' : '正常'}`
      : '等待有效帧…';
