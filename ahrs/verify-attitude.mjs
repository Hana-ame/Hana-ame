...
// 自适应增益 (仅自研版 lib/attitude.js 使用): 静止时大 Kp 快速收敛 + Ki 消零偏;
// 运动时降低, 避免线性加速度污染。ahrs 包 kp/ki 为构造常量, 运行期不可调, 故本文件不再使用。

console.log('== Mahony 融合跟踪 (ahrs 包, 固定增益, 陀螺噪声 + 恒定零偏, 20s) ==');
{
  const { angles, gyros, accels, dt } = simulateTrajectory();
  // 注: ahrs 包的 kp/ki 是构造常量, 运行期只能重建实例(会丢积分器/姿态),
  // 故 ahrs 版 UI 使用固定增益而非自适应(adaptiveGains 仅在自研版/verify 对照)。
  ...
  ok(`ahrs Mahony 融合(固定增益+零偏) 最大姿态误差 < 3°`, maxMah < 3, `max=${maxMah.toFixed(2)}°`);
