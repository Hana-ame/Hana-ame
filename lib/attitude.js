// 手机姿态解算核心数学 (纯 JS, 零依赖, 可在 node 与浏览器中直接使用)
//
// 由 test/attitude.js 抽出的独立包 (test/ 不再改动), 供其他 app 直接复用。
// 数学实现与 test/attitude.js 保持逐字一致, 二者共享同一套坐标系与算法。
//
...
//
// 快速上手:
//   import { quatFromDeviceEuler, qRotate } from 'lib/attitude.js';
//   const q = quatFromDeviceEuler(alpha, beta, gamma);   // 欧拉角 -> 四元数
//   const screenNormal = qRotate(q, { x: 0, y: 0, z: 1 }); // 屏幕法线在世界系的方向
//   // 或用 Mahony 融合 (deviceorientation + devicemotion 陀螺/重力):
//   const mah = new Mahony({ kp: 0.5, ki: 0.05 });
//   mah.update(dt, gyroRad, accelMs2);
